...
public class Main {
    ...
    public static void tractar_error(int error) {
        switch (error) {
            case -1:
                System.out.println("Error: ");
                break;
            case -2:
                System.out.println("Error: un dels parametres no esta inicialitzat correctament.");
                break;
            case -3:
                System.out.println("Error: s'ha escrit malament algun dels tipus de nodes existents.");
                break;
            case -4:
                System.out.println("Error: els numeros que s'han d'introduir no poden ser negatius.");
                break;
        }
    }
    ...
    public static void consultar_nodes() {
        ...
        while (opcio != 0) {
            ...
            if (opcio >= 1 && opcio <= 4) {
                int error = 0;
                switch (opcio) {
                    case 1: error = escriure_llista_nodes("AUTORS");       break;
                ...
                if (error != 0) tractar_error(error);
                    ...
                    error = controladorPresentacio.modificar_node(tipusNode, id, nouNom);
                    if (error != 0) tractar_error(error);
                ...
                if (error != 0) tractar_error(error);
                    ...
                    if (error != 0) System.out.println("El node amb identificador " + id + " s'ha eliminat correctament");
                    else tractar_error(error);
                }
            }
